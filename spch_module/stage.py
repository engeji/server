...
from typing import Iterable, Tuple, List, Union, Callable
...
class Stage(BaseStruct):
    ...
    def calc_stage_summary_in(self, mode:Mode, freq:float)->CompSummary:
            ...
            Returns:
                CompSummary: Показатель работы ступени
            """
            ...
            return CompSummary(self, mode, freq, volume_rate, comp_degree)
            
    def calc_stage_summary_out(self, mode:Mode, freq:float, method:str='brentq', is_in_border:bool=False)->Union[None, CompSummary]:
        cur_mode = mode.clone()
    ...
    def get_key_value(self, key:str, is_get_value:bool)->Callable[[Mode, float], CompSummary]:
        dispatcher_border = {
        ...
        return dispatcher_border[key][is_get_value]
    def get_freq_min_max_out(self, mode:Mode, border:Border)->Tuple[CompSummary,CompSummary]:
        ...
        for weight in filter(lambda x: x.key != 'percent_x', border):
            def calc(val):
                curr_summ:CompSummary = self.get_key_value(weight.key, True)(curr_mode, val)
                if curr_summ.percent_x < border['percent_x'].min_val:
                    return self.get_key_value(weight.key, False)(curr_mode, border['percent_x'].min_val )
                elif curr_summ.percent_x > border['percent_x'].max_val:
                    return self.get_key_value(weight.key, False)(curr_mode, border['percent_x'].max_val )
                else:
                    return curr_summ
            all_summs.append((
                calc(weight.min_val),
    ...
    def get_freq_dim_by_perc(self, mode:Mode, percent_x:float)->Union[None,CompSummary]:
    ...
    def get_comp_by_perc(self, mode:Mode, percent_x:float)->CompSummary:
        koef_raskh = self.type_spch.koef_raskh_by_percent_x(percent_x)
        kpd = self.type_spch.calc_k_kpd(koef_raskh)
        koef_nap = self.type_spch.calc_k_nap(koef_raskh)
        q_in = mode.q_in[self.idx] / self.w_cnt_current
        ...
        return summ
    def get_perc_by_freq_dim(self, mode:Mode, freq_dim:float)->Union[None,CompSummary]:
        return self.calc_stage_summary_out(mode, freq_dim * self.type_spch.fnom, is_in_border=True)
    def get_mght_by_perc(self, mode:Mode, percent_x:float)->CompSummary:
        koef_raskh = self.type_spch.koef_raskh_by_percent_x(percent_x)
        kpd = self.type_spch.calc_k_kpd(koef_raskh)
        koef_nap = self.type_spch.calc_k_nap(koef_raskh)
        q_in = mode.q_in[self.idx] / self.w_cnt_current
        ...
        summ = self.calc_stage_summary_in(curr_mode, freq)
        return summ
    def get_perc_by_mght(self, mode:Mode, mght:float)->CompSummary:
        q_in = mode.q_in[self.idx] / self.w_cnt_current
        mas_rate =  q_in * self.lim.plot_std * (10**6) / 24 / 60 / 60
        ...
        summ = self.calc_stage_summary_in(curr_mode, freq)
        return summ
    def get_perc_by_comp(self, mode:Mode, comp_degree:float)->CompSummary:
        ...
        return summ
    def show_plt(self, t_in:float, f_max:float, f_min:float, summ:Iterable[CompSummary]=None, isLine:bool=False, z_in=None):
