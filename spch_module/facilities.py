...
from .summary import CompSummary, CompSummaryCollection
from .discript import Header_list
...
def get_spch_by_name(name:str)->Spch:
    ...
    try:
        ...
        next(filter(lambda sp: sp.name == Header_list.type_spch.value['default'], ALL_SPCH_LIST))
...
def get_summ_collections(
    ...
    if isinstance(sums[0], CompSummary):
