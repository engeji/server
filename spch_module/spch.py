...
class Spch:           
    ...
    Bordered_koef_raskh = namedtuple("Bordered_koef_raskh", "koef min_freq")
    def koef_raskh_with_border(self, q_in:float, p_in:float, t_in:int)->:
    ...
    def _get_t_out(self, comp, t_in, k, kpd):
        mt = (k - 1)/ (k * kpd)
        return t_in * (comp ** mt)
    
    def get_sol_stage_by_mode_and_limit(self, freq, t_in, p_in, q, lim:Limit)->Solv:
        koef_raskh = self.koef_raskh(q, p_in, freq, t_in, lim.R, lim.plot)
        z_in = my_z(p_in, t_in)        
        volume_rate, cur_comp_deg = self.calc_xy(
            freq,
            koef_raskh,
            z_in,
            lim.R,
        ...
        t_out = self._get_t_out(cur_comp_deg, t_in, lim.k, kpd)
        z_out = my_z(cur_comp_deg * p_in, t_out)
        max_raskh = self._max_k_raskh
        min_raskh = self._min_k_raskh
        dh_val = dh(cur_comp_deg, (z_in + z_out) / 2, t_in, lim.R, lim.k, kpd)
        res =  Solv(
            freq=freq,
            freq_dim=freq / self.fnom,
            # mght = round(4 * lim.k / (lim.k - 1) * z_avg *(t_out - t_in) * q ),
            mght=round(dh_val * q * (10**6) * lim.plot /(24 * 60 * 60) / 1000 / kpd ),
            p_out_res=round(p_in * cur_comp_deg, 2),
            volume_rate=round(volume_rate),
            comp_degree=round(cur_comp_deg, 2),
            percent_x=int(100 * (koef_raskh - min_raskh) / (max_raskh - min_raskh)),
            kpd=kpd,
            t_out=t_out - 273.15
        )
        return res
